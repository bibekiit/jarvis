...
def _handle_event(name, string, format):
    ...
    if handler is not None:
        a = TokenString(string, tags=format)
        b = handler(a)
...
def parse(string, tokenize=True, tags=True, chunks=True, relations=True, anchors=True, lemmata=True, encoding=config.encoding):
    ...
    if lemmata or anchors:
        ...
        s = _handle_event("on_lemmatize", s, f)
    # Find PP anchors.
    if anchors:
...
class TokenString(unicode):
    ...
    def __new__(self, string, tags=[WORD]):
        ...
        """
        if isinstance(string, TokenString): tags = string.tags
...
class TokenList(list):
    ...
    def string(self):
        ...
        return TokenString(s, list(self.tags))
    ...
    def copy(self):
        return TokenList(self, self.tags)
