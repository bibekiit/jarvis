...
stack = ['routing','name','intent','welcome']
...
def print_path(x, unknown, known, pop_or_remove_condition,n,f):
    for i in range(0,len(x)):
        ...
        if pop_or_remove_condition != 'self.status_dict.pop()':
        ...
        if i==len(x)-1:
            ...
            tab_string3 = ''.join(['    ']*(i+n+2))
            print tab_string + "if "+x[i]+":"
            print tab_string2 + "if '"+x[i]+"' in self.status_dict:"
            print tab_string3 + pop_or_remove
            ...
            print tab_string2 + "return k.respond("+response+"')"
...
def print_path_all_possibilities_from_known(x, known,n,f):
    ...
    for i in range(0,len(x)):
        no_of_tabs = n+i
        if i==0:
            unknown += ' ' + print_path(x, unknown, known, 'self.status_dict.pop()',no_of_tabs,f)
        else:
            unknown += ' ' + print_path(x[i:], unknown, known, 'self.status_dict.remove',no_of_tabs,f)
def print_path_all_possibilities_from_given_Stack(x,f):
    for i in range(0,len(x)):
        if i==0:
            ...
            print "    " + "if self.status_dict[-1]=='"+x[len(x)-1-i]+"':"
            print "        " + "if '"+x[len(x)-1-i]+"' in self.status_dict:"
            print "            " + "self.status_dict.pop()"
            print_path_all_possibilities_from_known(new_stack,x[len(x)-1-i] + " done'",2,f)
            
        else:
            new_stack = x[0:len(x)-i]
            new_stack.reverse()
        
            print "    " + "elif self.status_dict[-1]=='"+x[len(x)-1-i]+"':"
