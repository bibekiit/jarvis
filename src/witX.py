# -*- coding: utf-8 -*-
"""
Created on Tue Mar  1 15:13:14 2016
...
import mbsp as MBSP, nltk,re, os
...
class witX():
    ...
    def getAgendaV2(self, TEXT):
        ...
        if TEXT != "":
            ...
            t = TEXT            
            s = MBSP.parse(t)
            subj_list = [i[0].encode('ascii','ignore') for i in s.split()[0] if i[4]!='O' and 'SBJ' in i[4].encode('ascii','ignore')]
            subj = ' '.join(subj_list)
            #print subj
            intent_obj_list = [(i[4].split('-')[-1].encode('ascii','ignore'),i[0].encode('ascii','ignore')) for i in s.split()[0] if i[4]!='O' and 'SBJ' not in i[4].encode('ascii','ignore')]    
            probable_intent = [[elem for _, elem in group][0] for key, group in groupby(intent_obj_list, lambda pair: pair[0])]
    ...
    def getAmount(self, TEXT):
        if TEXT!= '':
            ...
            s = MBSP.parse(TEXT).split()[0]
            amount = [i[0] for i in s  if i[1]=='CD' and len(i[0])<=4]
            if len(amount)>0:
...
class main():
    def test(self,msg,category,lang):
        gs = goslate.Goslate()
        translated_msg = gs.translate(msg, lang)
        w = witX()
        return w.getEntities(translated_msg,category)
